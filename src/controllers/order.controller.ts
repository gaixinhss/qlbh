...
import { createOrder, deleteOrder, getAllOrder, getLastOrder, getOrderByCode,revenueByCustomer,revenueByProduct, updateOrder } from '../services/order.service'
...
export const createOrd  = async(req:express.Request,res:express.Response)=>{
    try{
        ...
        orderItem.forEach(async(item)=>{//duyệt từng sản phẩm trong order item
            let productCode:string = item.product;
            let product = await getProductByCode(productCode);//lấy thông tin product
            if(!product){
                throw new ApiError(httpStatus.NOT_FOUND,"product not found" + productCode)
            }
            let inventory:number = product?.inventory!;//số lượng tồn kho
           
            if(inventory<item.quantity){//kiểm tra số lượng mua
                throw  new ApiError(httpStatus.BAD_REQUEST,"too many products "+product?.name)
            }
            item.price = product?.sellingPrice!;//lâấy giá bán hiện tại
            
            total+=item.quantity*item.price;//tính tổng tiền
            
            //update số lượng sản phẩm
            let body: any={inventory:inventory-item.quantity};
    ...
    try{
       ...
       const  orderItem = order.orderItem;
        orderItem.forEach(async(item)=>{
            let productCode:string = item.product!;
            let inventory:number =(await getProductByCode(productCode))?.inventory!;
