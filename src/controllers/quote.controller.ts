...
import { createQuote, deleteQuote, findQuote, getAllQuote, getLastQuote, getQuoteByCode, updateQuote } from '../services/quote.service';
...
export const createQuo = async(req:express.Request,res:express.Response)=>{
    try{
        ...
        if(!existCustomer){//kiểm tra tồn tại của customer
            throw new ApiError(httpStatus.NOT_FOUND,"customer not found");
        }
        const existEmployee =await getUserByCode(employee);
        if(!existEmployee){//kiểm tra tồn tại employee
