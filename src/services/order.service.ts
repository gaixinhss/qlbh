...
export const getQuantity=(code:string,codeProduct:string)=>orderModel.aggregate([
    {
      $match: {
        code: code,
      },
  ...
  export const revenueByProduct =()=>orderModel.aggregate([
    {
      $unwind: "$orderItem",
    },
    {
  ...
  export const revenueByCustomer=()=>orderModel.aggregate([
    {
      $group: {
        _id: "$customer",
...
export const updateOrder=(code:string,values:Record<string,any>)=>orderModel.findOneAndUpdate({code},values,{new:true});
...
export const replaceOrderItem=(code:string,orderItemNew:any)=>orderModel.updateOne({code:code},{$set:{orderItem:orderItemNew}})
