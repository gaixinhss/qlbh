...
export const createQuote = (values: Record<string, any>) =>
  new quoteModel(values).save().then((quote) => quote.toObject())
...
export const updateQuote = (code: string, value: Record<string, any>) =>
  quoteModel.findOneAndUpdate({ code }, value, { new: true })
